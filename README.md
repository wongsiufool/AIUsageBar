...
「应用程序」即可。应用未经 Apple 公证（无开发者账号签名），首次打开若被 Gatekeeper
拦截：右键点击应用 → 打开，或在「系统设置 → 隐私与安全性」中点「仍要打开」。
首次运行会请求读取钥匙串中 Claude Code 的凭证——输入登录密码并点「始终允许」。
...
⚠️ 每次重新构建后二进制签名（ad-hoc）会变化，首次启动时 macOS 会重新弹出
钥匙串授权对话框——输入登录密码并点击「始终允许」即可，同一二进制之后不再询问。

开机自启：系统设置 → 通用 → 登录项，把 `build/AIUsageBar.app` 加入即可。
